...
export const RadioGroupContext = React.createContext<IRadioGroupContext>({
  ...
  onRefChange: (/* ref: React.RefObject<any>*/) => {
    return;
  },
});
...
export const RadioGroup = compose<IRadioGroupType>({
  ...
  usePrepareProps: (userProps: IRadioGroupProps, useStyling: IUseComposeStyling<IRadioGroupType>) => {
    ...
    const [selectedKeyRef, setSelectedKeyRef] = React.useState(React.useRef<View>(null));

    const onSelectKeyRef = React.useCallback(
      (ref: React.RefObject<View>) => {
        setSelectedKeyRef(ref);
        console.log(ref);
      },
      [setSelectedKeyRef],
    );
    ...
    const slotProps = mergeSettings<IRadioGroupSlotProps>(styleProps, {
      ...
      container: { isCircularNavigation: true, defaultTabbableElement: selectedKeyRef },
    });
